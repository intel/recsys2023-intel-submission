...
def main(args):
    ...
    # print("-"*100)
    # print(config['edge_split'])
    # #df.loc[df.index[-1], 'test_mask'] = 1 # done for engineering purpose, so that following block of code works
    # print("-"*100)
    ...
    for i, edge_type in enumerate(meta_yaml["edge_data"]):
        ...
        if config["edge_split"]:
            ...
            edge_df_cols.extend(["train_mask", "val_mask", "test_mask"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_in",
        type=file,
        default="/localdisk/akakne/recsys2023/data/supGNN_graph_data/train_graph/train_edges.csv.gz",
        help="Input file with path (processed_data.csv) ",
    )
    parser.add_argument(
        "--gnn_tmp", default="/localdisk/akakne/recsys2023/data/supGNN_graph_data/train_graph", 
        help="The path to the gnn_tmp"
    )
    parser.add_argument(
        "--tab2graph_cfg", default="/localdisk/akakne/recsys2023/0_train/2_supGNN/recsys2graph.yaml", 
        help="The path to the tabular2graph.yaml"
    )
    parser.add_argument("--CSVDataset_name", type=str, default="recsys_graph")
