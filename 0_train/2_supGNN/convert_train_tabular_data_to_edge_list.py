...
parser.add_argument( "--output_file", "-o", default="/localdisk/akakne/recsys2023/data/supGNN_graph_data/train_graph/train_edges.csv.gz")
...
train_val_df = train_val_df.replace('', np.nan)

for col in categorical_features:
    if (train_val_df[col].isnull().values.any()):
        train_val_df[col] = train_val_df[col].fillna(-1)
...
for col in numerical_features:
    if (train_val_df[col].isnull().values.any()):
        train_val_df[col] = train_val_df[col].replace(np.NaN, train_val_df[col].mean()) 

if normalize:
    print("WARNING: Using normalization of numerical features")
    mean =  train_val_df[numerical_features].mean()
    std = train_val_df[numerical_features].std()
    train_val_df[numerical_features] = (train_val_df[numerical_features] - mean) / std
...
role1.sort()
role2.sort()
role3 = list(set(train_val_df.columns) - set(role1) - set(role2) - set(labels) )
role3.sort()

role_1_df = train_val_df.drop_duplicates(subset=role1).reset_index()[role1]
role_2_df = train_val_df.drop_duplicates(subset=role2).reset_index()[role2]
...
train_val_graph_df = get_group_idx(train_val_df, 1, role1)
train_val_graph_df = get_group_idx(train_val_graph_df, 2, role2)
train_val_graph_df.rename(columns={"group_1": "src_id", "group_2": "dst_id"}, inplace=True)

# step 10 : write processed graph dataset to file
chunks = np.array_split(train_val_graph_df.index, 100) # split into 100 chunks for easy progress tracking
desired_cols = ['src_id', 'dst_id'] + role1 + role2 + role3 + labels

for chunk, subset in enumerate(tqdm(chunks, desc='Writing csv file')):
    chunk_df = train_val_graph_df[desired_cols].loc[subset]
