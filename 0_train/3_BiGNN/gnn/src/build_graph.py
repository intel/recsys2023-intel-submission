...
def main(args):
    ...
    start = time.time()
    df = pd.read_csv(args.data_in, compression='gzip', engine='pyarrow')#, nrows=100000)
    ...
    if config["edge_split"]:
        df = pd.concat(
        [df, pd.get_dummies(df[config["edge_split"]].astype("category"), prefix="masks")],
        axis=1,)
    ...
    for i, node in enumerate(col_map.keys()):
        list_of_n_dict.append({"file_name": "nodes_"+str(i)+".csv", "ntype":col_map[node]})
    ...
    for i, edge_type in enumerate(config['edge_types']):
        ...
        list_of_e_dict.append({"file_name": "edges_"+str(i)+".csv", "etype":edge_type})
  
    with open(os.path.join(CSVDataset_dir, "meta.yaml"), "w") as f:   
        python_data={"dataset_name":args.CSVDataset_name,
    ...
    for i, edge_type in enumerate(meta_yaml['edge_data']):
        ...
        if config["edge_split"]:
            ...
            if len(config["edge_split"]) == 1:
                edge_df_cols.extend(["masks_0", "masks_1","masks_2"])
            elif len(config["edge_split"]) == 3:
                edge_df_cols.extend(config["edge_split"])
            else:
                print("Not including train/test/val split. input needs to be 1 column values (0,1,2) or 3 binary mask columns")
    ...
    for i, node in enumerate(meta_yaml['node_data']):
        ...
        node_header=["node_id"] # minimum required
        node_df=df.drop_duplicates(meta_yaml['node_data'][i]["ntype"])
        node_df_cols=[meta_yaml['node_data'][i]["ntype"]]
        assert len(node_df_cols) == len(node_header)
        print(node_header)
        node_df[node_df_cols].to_csv(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--CSVDataset_name", type=str, help="CSVDataset name"
    )
