# Copyright (C) 2023 Intel Corporation
# SPDX-License-Identifier: MIT
# Downloaded from https://github.com/intel/graph-neural-networks-and-analytics
import torch
import torch.nn as nn
import torch.nn.functional as F
import torchmetrics.functional as MF

import dgl
import dgl.nn as dglnn
from dgl.nn import SAGEConv, EGATConv
from dgl.nn import EdgePredictor
from dgl.dataloading import (
    DataLoader,
    NeighborSampler,
    MultiLayerFullNeighborSampler,
    as_edge_prediction_sampler,
    negative_sampler,
)
from dgl import save_graphs, load_graphs
from dgl import DGLGraph

import tqdm
import argparse
import time
from sklearn.metrics import roc_auc_score
import numpy as np
import random

use_node_feat = False
use_label_as_feat = False
use_only_isinstalled_nodes = True

def to_bidirected_with_reverse_mapping(g):
    """Makes a graph bidirectional, and returns a mapping array ``mapping`` where ``mapping[i]``
    is the reverse edge of edge ID ``i``. Does not work with graphs that have self-loops.
    """
    g_simple, mapping = dgl.to_simple(
        dgl.add_reverse_edges(g), return_counts="count", writeback_mapping=True
    )
    c = g_simple.edata["count"]
    num_edges = g.num_edges()  # original graph
    # mapping is size number of edges of dgl.add_reverse_edges(g)
    # dgl.add_reverse_edges(g) appends the reverse edges, so the second half of ids are the reverse edges
    # For a graph with edges (𝑖1,𝑗1),⋯,(𝑖𝑛,𝑗𝑛), this function creates a new graph with edges (𝑖1,𝑗1),⋯,(𝑖𝑛,𝑗𝑛),(𝑗1,𝑖1),⋯,(𝑗𝑛,𝑖𝑛)
    dir_eidx = np.unique(
        mapping[0 : num_edges - 1]
    )  # ids of all directed edges in g_simple
    rev_eidx = np.unique(mapping[num_edges:-1])  # ids of all reverse edges in g_simple
    assert len(dir_eidx) + len(rev_eidx) == g_simple.num_edges()
    mapping_offset = torch.zeros(g_simple.num_edges() + 1, dtype=g_simple.idtype)
    mapping_offset[1:] = c.cumsum(0)
    idx = (
        mapping.argsort()
    )  # print(mapping[idx]) will show sorted #same size as mapping
    idx_uniq = idx[mapping_offset[:-1]]
    reverse_idx = torch.where(
        idx_uniq >= num_edges, idx_uniq - num_edges, idx_uniq + num_edges
    )
    reverse_mapping = mapping[reverse_idx]
    assert len(reverse_mapping) == g_simple.num_edges()
    # sanity check
    src1, dst1 = g_simple.edges()
    src2, dst2 = g_simple.find_edges(reverse_mapping)
    assert torch.equal(src1, dst2)
    assert torch.equal(src2, dst1)
    return g_simple, reverse_mapping, dir_eidx

class GraphSAGE(nn.Module):
    def __init__(
        self, in_feats, hidden_size, out_feats, n_layers, activation, aggregator_type
    ):
        super(GraphSAGE, self).__init__()
        self.layers = nn.ModuleList()
        self.activation = activation

        # input layer
        self.layers.append(SAGEConv(in_feats, hidden_size, aggregator_type))
        # hidden layers
        for i in range(1, n_layers - 1):
            self.layers.append(SAGEConv(hidden_size, hidden_size, aggregator_type))
        # output layer
        self.layers.append(
            SAGEConv(hidden_size, out_feats, aggregator_type)
        )  # activation None

    def forward(self, graphs, inputs):
        h = inputs
        for l, (layer, block) in enumerate(zip(self.layers, graphs)):
            #breakpoint()
            h = layer(block, h)
            if l != len(self.layers) - 1:
                h = self.activation(h)
        return h 


class Model(nn.Module):
    def __init__(self, vocab_size, in_feat, hid_size, n_layers):
        super().__init__()

        self.hid_size = hid_size

        # node embedding
        self.emb = torch.nn.Embedding(vocab_size, hid_size)
        # encoder is a 1-layer GraphSAGE model
        self.encoder = GraphSAGE(in_feat, hid_size, hid_size, n_layers, F.relu, "mean")
        # decoder is a 3-layer MLP
        if use_label_as_feat:
            self.decoder = nn.Sequential(
                nn.Linear(hid_size+1, hid_size),
                nn.ReLU(),
                nn.Linear(hid_size, hid_size),
                nn.ReLU(),
                nn.Linear(hid_size, 1),
            )

        else:
            self.decoder = nn.Sequential(
                nn.Linear(hid_size, hid_size),
                nn.ReLU(),
                nn.Linear(hid_size, hid_size),
                nn.ReLU(),
                nn.Linear(hid_size, 1),
            )
        #cosine similarity with linear
        self.predictor=dglnn.EdgePredictor('cos')
        #self.predictor=dglnn.EdgePredictor('cos',hid_size,1)

    def forward(self, pair_graph, neg_pair_graph, blocks, x):
        if use_node_feat == True:
            h = x
            
        else:
            h = self.emb(x)
        h = self.encoder(blocks, h)

        pos_src, pos_dst = pair_graph.edges()
        neg_src, neg_dst = neg_pair_graph.edges()
        
        if use_label_as_feat == True:
            # breakpoint()
            labels = pair_graph.edata['label'].reshape((-1, 1))
            h_pos = self.decoder(torch.hstack((h[pos_src] * h[pos_dst], labels)))
            h_neg = self.decoder(torch.hstack((h[neg_src] * h[neg_dst], 0.1*torch.ones_like(labels))))
        
        else:
            h_pos = self.predictor(h[pos_src], h[pos_dst])
            h_neg = self.predictor(h[neg_src], h[neg_dst])
            # h_pos = self.decoder(h[pos_src] * h[pos_dst])
            # h_neg = self.decoder(h[neg_src] * h[neg_dst])
        return h_pos, h_neg

    def inference(self, g, device, batch_size):
        """Layer-wise inference algorithm to compute GNN node embeddings."""
        if use_node_feat==True:
            feat = g.ndata['feat']
        else:
            feat = self.emb.weight.data
        #use pretrained embedding as node features
        #feat = self.emb.weight.data
        # sampler = MultiLayerFullNeighborSampler(1)
        vocab_size = len(self.emb.weight) 
        sampler = dgl.dataloading.NeighborSampler(fanouts=[-1],prob='prob')
        dataloader = DataLoader(
            g, torch.arange(g.num_nodes()).to(g.device), sampler, device=device,
            batch_size=batch_size, shuffle=False, drop_last=False,
            num_workers=4)
        buffer_device = torch.device('cpu')
        pin_memory = (buffer_device != device)
        #compure representations layer by layer
        for l, layer in enumerate(self.encoder.layers):
            y = torch.empty(g.num_nodes(), self.hid_size, device=buffer_device,
                            pin_memory=pin_memory)
            feat = feat.to(device)
            #within a layer iterate over nodes in batches
            with dataloader.enable_cpu_affinity():
                for input_nodes, output_nodes, blocks in tqdm.tqdm(dataloader, desc='Inference'):
                    h = []
                    for node in input_nodes:
                        # If the node belongs to training data, get it's emb from the trained GNN model
                        if node < vocab_size:
                            h_node = feat[node]
                        # Else, it's an unseen node. Thus, will have a random embedding. 
                        else:
                            h_node = torch.rand(self.hid_size)
                        h += h_node,
                    h = torch.stack(h, dim=0)
                    h = layer(blocks[0], h)
                    if l != len(self.encoder.layers) - 1:
                        h = F.relu(h)
                    y[output_nodes] = h
                feat = y
        return y                    
                    
        #             x = feat[input_nodes]
        #             h = layer(blocks[0], x)
        #             if l != len(self.encoder.layers) - 1:
        #                 h = F.relu(h)
        #             y[output_nodes] = h.to(buffer_device)
        #         feat = y
        # return y

def evaluate(model, test_dataloader):
    model.eval()
    score_all = torch.tensor(())
    labels_all = torch.tensor(())
    with test_dataloader.enable_cpu_affinity():
        with torch.no_grad():
            for it, (input_nodes, pair_graph, neg_pair_graph, blocks) in enumerate(
                tqdm.tqdm(test_dataloader)
            ):
                #x = blocks[0].srcdata[dgl.NID]
                if use_node_feat==True:
                    x = blocks[0].srcdata["feat"]
                else:
                    x = blocks[0].srcdata[dgl.NID]

                pos_score, neg_score = model(pair_graph, neg_pair_graph, blocks, x)
                score = torch.cat([pos_score, neg_score])
                pos_label = torch.ones_like(pos_score)
                neg_label = torch.zeros_like(neg_score)
                labels = torch.cat([pos_label, neg_label])
                score_all = torch.cat([score_all, score])
                labels_all = torch.cat([labels_all, labels])
            rocauc = roc_auc_score(labels_all, score_all)
    return rocauc


def train(args, device, g, train_dataloader, val_dataloader, test_dataloader, model):
    best_rocauc = 0
    best_model_path = args.model_out
    print("learning rate: ", args.lr)
    opt = torch.optim.Adam(model.parameters(), lr=args.lr)
    with train_dataloader.enable_cpu_affinity():
        for epoch in range(args.num_epochs):
            start = time.time()
            step_time = []
            model.train()
            total_loss = 0
            total_val_loss=0.0
            for it, (input_nodes, pair_graph, neg_pair_graph, blocks) in enumerate(
                train_dataloader
            ):
                #x = blocks[0].srcdata[dgl.NID] #this is equal to input_nodes
                if use_node_feat==True:
                    x = blocks[0].srcdata["feat"]
                    e = blocks[0].edata["feat"]
                else:
                    x = input_nodes
                pos_score, neg_score = model(pair_graph, neg_pair_graph, blocks, x)
                #pos_score, neg_score = model(pair_graph, neg_pair_graph, blocks, input_nodes)
                score = torch.cat([pos_score, neg_score])
                pos_label = torch.ones_like(pos_score)
                neg_label = torch.zeros_like(neg_score)
                labels = torch.cat([pos_label, neg_label])
                loss = F.binary_cross_entropy_with_logits(score, labels)
                opt.zero_grad()
                loss.backward()
                opt.step()
                total_loss += loss.item()
                step_t = time.time() - start
                step_time.append(step_t)
                start = time.time()
            #print("Epoch {:05d} | Train Loss {:.4f}".format(epoch, total_loss / (it + 1)))
            #torch.save(model.state_dict(), best_model_path)

            model.eval()
            
            with val_dataloader.enable_cpu_affinity():
                for it2, (input_nodes, pair_graph, neg_pair_graph, blocks) in enumerate(val_dataloader):
                    # x = blocks[0].srcdata[dgl.NID] this is same as input_nodes
                    if use_node_feat==True:
                        x = blocks[0].srcdata["feat"]
                    else:
                        x = input_nodes
                    pos_score, neg_score = model(pair_graph, neg_pair_graph, blocks, x)
                    score = torch.cat([pos_score, neg_score])
                    pos_label = torch.ones_like(pos_score)
                    neg_label = torch.zeros_like(neg_score)
                    labels = torch.cat([pos_label, neg_label])
                    val_loss = F.binary_cross_entropy_with_logits(score, labels)
                    total_val_loss += val_loss.item()
                print("Epoch {:05d} | Epoch time {:.4f} | Train Loss {:.4f} | Valid Loss {:.4f}".format(epoch, np.sum(step_time),float(total_loss) / (it + 1) ,float(total_val_loss) / (it2 + 1)))

            if (epoch % args.eval_every == 0 and epoch != 0) or (epoch == args.num_epochs -1):
                rocauc = evaluate(model, test_dataloader)
                # update best model if needed
                if best_rocauc < rocauc:
                    print("Updating best model")
                    best_rocauc = rocauc
                    torch.save(model.state_dict(), best_model_path)
                print(
                    "Epoch {:05d} | Test roc_auc_score {:.4f}".format(
                        epoch, rocauc
                    )
                )


def main(arglist):
    # random seeds for testing
    seed=7
    print("random seed set to: ",seed)
    random.seed(seed)
    np.random.seed(seed)
    dgl.seed(seed)
    dgl.random.seed(seed)
    torch.random.manual_seed(seed)
    torch.manual_seed(seed)

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--mode",
        default="cpu",
        choices=["cpu", "gpu", "mixed"],
        help="Training mode. 'cpu' for CPU training, 'gpu' for pure-GPU training, "
        "'mixed' for CPU-GPU mixed training.",
    )
    parser.add_argument(
        "--CSVDataset_dir",
        default="/GNN_TMP/sym_tabformer_hetero_CSVDataset/",
        help="Path to CSVDataset",
    )
    parser.add_argument(
        "--model_out",
        default="/GNN_TMP/sym_tabformer_hetero_CSVDataset/model_graphsage_2L_64.pt",
        type=str,
        help="output for model /your_path/model_graphsage_2L_64.pt",
    )
    parser.add_argument(
        "--nemb_out",
        default="/GNN_TMP/sym_tabformer_hetero_CSVDataset/node_emb.pt",
        type=str,
        help="node emb output: /your_path/node_emb.pt",
    )
    parser.add_argument("--num_epochs", type=int, default=3)
    parser.add_argument("--num_hidden", type=int, default=16)
    parser.add_argument("--num_layers", type=int, default=2)
    parser.add_argument("--fan_out", type=str, default="10,25")
    parser.add_argument("--batch_size", type=int, default=1000)
    parser.add_argument("--batch_size_eval", type=int, default=100000)
    parser.add_argument("--eval_every", type=int, default=5)
    parser.add_argument("--lr", type=float, default=0.003)

    #    args = parser.parse_args()
    args = parser.parse_args(arglist)
    if not torch.cuda.is_available():
        args.mode = "cpu"
    print(f"Training in {args.mode} mode.")

    # load and preprocess Tabformer dataset
    print("Loading data")
    start = time.time()
    # set force_reload=False if no changes on input graph (much faster otherwise ingestion ~30min)
    dataset = dgl.data.CSVDataset(args.CSVDataset_dir, force_reload=True)
    print("time to load tabformer from CSVs: ", time.time() - start)

    hg = dataset[0]  # only one graph
    print(hg)
    print("etype to read train/test/val from: ",hg.canonical_etypes[0][1])
    
    if use_only_isinstalled_nodes:
        train_mask = torch.logical_and((hg.edges[hg.canonical_etypes[0][1]].data["label"]>0.1).bool(), hg.edges[hg.canonical_etypes[0][1]].data["train_mask"].bool())
        val_mask = torch.logical_and((hg.edges[hg.canonical_etypes[0][1]].data["label"]>0.1).bool(), hg.edges[hg.canonical_etypes[0][1]].data["val_mask"].bool())
        test_mask = hg.edges[hg.canonical_etypes[0][1]].data["test_mask"]
    else:
        train_mask = hg.edges[hg.canonical_etypes[0][1]].data["train_mask"]
        val_mask = hg.edges[hg.canonical_etypes[0][1]].data["val_mask"]
        test_mask = hg.edges[hg.canonical_etypes[0][1]].data["test_mask"]
    train_eidx = torch.nonzero(train_mask, as_tuple=False).squeeze()
    val_eidx = torch.nonzero(val_mask, as_tuple=False).squeeze()
    test_eidx = torch.nonzero(test_mask, as_tuple=False).squeeze()
    
    #Removing test edges
    big_hg = hg
    gg = dgl.remove_edges(hg, eids=test_eidx, etype=('src_id', 'e', 'dst_id'))
    hg = dgl.remove_edges(gg, eids=test_eidx, etype=('dst_id', 'sym_e', 'src_id'))
    train_mask = torch.logical_and((hg.edges[hg.canonical_etypes[0][1]].data["label"]>0.1).bool(), hg.edges[hg.canonical_etypes[0][1]].data["train_mask"].bool())
    val_mask = torch.logical_and((hg.edges[hg.canonical_etypes[0][1]].data["label"]>0.1).bool(), hg.edges[hg.canonical_etypes[0][1]].data["val_mask"].bool())
    train_eidx = torch.nonzero(train_mask, as_tuple=False).squeeze()
    val_eidx = torch.nonzero(val_mask, as_tuple=False).squeeze()
    
    # train_mask = torch.cat((train_eidx, val_eidx, test_eidx))
    # val_mask = torch.cat((train_eidx, val_eidx, test_eidx))

    E = hg.num_edges(hg.canonical_etypes[0][1])
    reverse_eids = torch.cat([torch.arange(E, 2 * E), torch.arange(0, E)])
    print("First reverse id is:  ", reverse_eids[0])
    if use_node_feat==True:
        x=torch.hstack((hg.nodes['dst_id'].data['feat'],torch.zeros(hg.nodes['dst_id'].data['feat'].shape[0],hg.nodes['src_id'].data['feat'].shape[1]))) 
        y=torch.hstack((torch.zeros(hg.nodes['src_id'].data['feat'].shape[0],hg.nodes['dst_id'].data['feat'].shape[1]), hg.nodes['src_id'].data['feat']))
        hg.nodes['src_id'].data['feat']=y
        hg.nodes['dst_id'].data['feat']=x
    
        g = dgl.to_homogeneous(hg, ndata=['feat'], edata=['feat', 'label'])
    elif use_label_as_feat==True:
        g = dgl.to_homogeneous(hg, edata=['label'])
    else:
        g = dgl.to_homogeneous(hg)
        big_g = dgl.to_homogeneous(big_hg, edata=['test_mask'])
        big_g.edata['prob'] = 1- big_g.edata['test_mask']
    g = g.to("cuda" if args.mode == "gpu" else "cpu")
    device = torch.device("cpu" if args.mode == "cpu" else "cuda")

    train_eidx.to(device)
    val_eidx.to(device)
    test_eidx.to(device)

    vocab_size = g.num_nodes()


    use_uva = args.mode == "mixed"

    model = torch.load(args.model_out)
    vocab_size = len(model['emb.weight']) 


    hid_size = 64  # we are using same for embedding size and the hidden feature size of SAGEConv layers
    if use_node_feat==True:
        model = Model(vocab_size, 4, args.num_hidden, args.num_layers).to(device)
    else:
        model = Model(vocab_size, args.num_hidden, args.num_hidden, args.num_layers).to(device)


    # to load pretrained model if you just want to do inference
#    model.load_state_dict(torch.load(args.model_out))
#    node_emb = model.emb.weight.data
#    torch.save(node_emb, args.nemb_out)

    print("Inference to generate node representations...")
    # model.eval()
    model.load_state_dict(torch.load(args.model_out))
    node_emb = model.inference(big_g, device, args.batch_size_eval)
    print("node emb shape: ", node_emb.shape)
    torch.save(node_emb, args.nemb_out)

    # roc_auc on the different splits after training completion
    #rocauc_test = evaluate(model, test_dataloader)
    #print("final test rocauc", rocauc_test)



import sys

if __name__ == "__main__":
    main(sys.argv[1:])
