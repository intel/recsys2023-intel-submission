...
if [ "$#" -ne 5 ]
then
  echo "Incorrect number of arguments to run_inference_single.sh"
  exit 1
fi
...
EXEC_SCRIPT=${WORKSPACE}/src/graphsage_fraud_transductive_test_onebyone_inference.py
...
mkdir -p ${OUT_DIR}
MODEL_DIR=../best_embedding/
MODEL_OUT=${MODEL_DIR}/model_graphsage_2L_64.pt
NEMB_OUT=${MODEL_DIR}/node_emb.pt #these are the default names
NEMB_OUT_MAPPED=${MODEL_DIR}/node_emb_mapped.pt #these are the default names
OUT_DATA=${MODEL_DIR}/tabular_with_gnn_emb.csv
...
python -u ${EXEC_SCRIPT} --num_epoch ${EPOCHS}  --num_hidden ${HIDDEN_FS} --num_layers ${N_LAYERS} --lr ${L_RATE} --fan_out ${FANOUT} --batch_size ${MB_SIZE} --batch_size_eval ${MB_SIZE_EVAL} --eval_every ${EVAL_EVERY} --CSVDataset_dir ${CSVDATASET} --model_out ${MODEL_OUT} --nemb_out ${NEMB_OUT} |& tee ${logdir}/${logname}.txt